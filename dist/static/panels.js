...
function renderWorkspaceDropdownInto(dd, workspaces, currentWs){
  ...
  const machineDefaults = [
    { name: 'ubuntu', path: '~', _machine: 'ubuntu' },
    { name: 'pop! os', path: '~', _machine: 'popos' }
  ];
...
const REMOTE_COMPUTERS=[
  {id:'ubuntu',name:'ubuntu home',color:'rgba(59,130,246,0.08)',border:'rgba(59,130,246,0.3)',ip:'127.0.0.1',home:'~'},
  {id:'popos',name:'pop! os home',color:'rgba(244,174,17,0.08)',border:'rgba(244,174,17,0.3)',ip:'127.0.0.1',home:'~'},
];
...
#
# Quick fix:
# 1. Ensure you can SSH to ${window.MEMSTER_HOST || window.UBUNTU_IP || '127.0.0.1'}: ssh ${window.MEMSTER_USER || 'user'}@${window.MEMSTER_HOST || window.UBUNTU_IP || '127.0.0.1'}
# 2. If prompted for password, set up key auth: ssh-copy-id -i ${window.SSH_KEY_PATH || '~/.ssh/id_ed25519'} ${window.MEMSTER_USER || 'user'}@${window.MEMSTER_HOST || window.UBUNTU_IP || '127.0.0.1'}
# 3. Or generate a new key: ssh-keygen -t ed25519 -f ~/.ssh/id_ed25519
#
# Manual fallback:
# You can manually edit config.yaml on your Hermes host at:
