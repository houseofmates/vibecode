...
async function loadCrons() {
  ...
  if (!_cronsCache) {
    const cached = localStorage.getItem('hermes-crons-cache');
    if (cached) {
      try {
...
async function loadSkills() {
  ...
  if (!_skillsData) {
    const cached = localStorage.getItem('hermes-skills-cache');
    if (cached) {
      try {
...
async function loadWorkspaceList(){
  // Show cached workspaces instantly while fetching fresh data
  if (!_workspaceListCache) {
    const cached = localStorage.getItem('hermes-workspaces-cache');
    if (cached) {
      try {
        _workspaceListCache = JSON.parse(cached);
      } catch (e) { /* ignore parse errors */ }
    }
  }
  try{
...
async function loadProfilesPanel() {
  ...
  if (!_profilesCache) {
    const cached = localStorage.getItem('hermes-profiles-cache');
    if (cached) {
      try {
