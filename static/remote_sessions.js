...
(function() {
  ...
  function _loadDeletedSessions() {
    try {
      ...
      if (stored) {
        ...
        parsed.forEach(id => _recentlyDeleted.add(id));
      }
    } catch (e) {
  ...
  function _saveDeletedSessions() {
    try {
      localStorage.setItem(DELETED_STORAGE_KEY, JSON.stringify([..._recentlyDeleted]));
    } catch (e) {
  ...
  function wrapRenderSessionList() {
    ...
    if (typeof window.renderSessionList === 'function' && !window._originalRenderSessionList) {
      ...
      window.renderSessionList = async function() {
        ...
        // Fetch remote FIRST (parallel with local)
        const remoteFetch = fetchRemoteSessions().then(remote => {
        ...
        await remoteFetch;
        // Merge remote sessions into the unified list
        mergeRemoteSessions();
        return result;
      };
  ...
  function mergeRemoteSessions() {
    ...
    if (added > 0 && typeof renderSessionListFromCache === 'function' && !_renamingSid) {
  ...
  async function forceRefresh() {
    showToast('Refreshing remote sessions...');
    ...
    showToast('Remote sessions refreshed');
    return sessions;
  }
