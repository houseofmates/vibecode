...
Connects to .250 (192.168.4.250) for memster, wiki, and Simply Plural data via SSH
...
def list_memster_memories(category=None, tier=None, limit=50, offset=0, search=None):
    """List memories from memster on .250 with filtering."""
    query = "SELECT json_group_array(json_object("
    query += "'id', id, 'content', content, 'category', category, 'tier', tier, 'importance', importance, "
    query += "'created_at', created_at, 'accessed_at', accessed_at, "
    query += "'tags', COALESCE((SELECT json_group_array(tag) FROM memory_tags WHERE memory_id = memories.id), json_array()))) "
    ...
    if search:
        ...
        query += f" AND (content LIKE '%{search_escaped}%' OR EXISTS (SELECT 1 FROM memory_tags mt WHERE mt.memory_id = memories.id AND mt.tag LIKE '%{search_escaped}%'))"
...
def get_memster_tags():
    ...
    sql = "SELECT json_group_array(DISTINCT tag) FROM memory_tags"
    cmd = f"sqlite3 '{MEMSTER_DB}' '{sql}'"
    result = ssh_json_command(MEMSTER_HOST, MEMSTER_USER, cmd)
    try:
        data = json.loads(result.get('output', '[]'))
...
def search_memster_memories(query_text, limit=20):
    ...
    sql += "'id', id, 'content', content, 'category', category, 'importance', importance, 'created_at', created_at, "
    sql += "'tags', COALESCE((SELECT json_group_array(tag) FROM memory_tags WHERE memory_id = memories.id), json_array()))) "
    sql += f"FROM memories WHERE content LIKE '%{search_escaped}%' "
    sql += f"OR EXISTS (SELECT 1 FROM memory_tags mt WHERE mt.memory_id = memories.id AND mt.tag LIKE '%{search_escaped}%') "
...
def get_memster_briefing():
    ...
    now_sql = "SELECT json_object('id', id, 'content', content, 'category', category, 'timestamp', created_at) "
    now_sql += "FROM memories ORDER BY accessed_at DESC LIMIT 1"
    ...
    ctx_sql += "'timestamp', created_at)) FROM (SELECT * FROM memories ORDER BY accessed_at DESC LIMIT 30)"
    ...
    recent_sql = "SELECT count(*) FROM memories WHERE created_at > datetime('now', '-1 day')"
...
def get_memster_memory_by_id(mem_id):
    ...
    sql += "'importance', importance, 'created_at', created_at, 'accessed_at', accessed_at, "
    sql += "'tags', COALESCE((SELECT json_group_array(tag) FROM memory_tags WHERE memory_id = id), json_array()), "
    sql += f"'entities', COALESCE((SELECT json_group_array(json_object('name', name, 'type', type)) FROM entities WHERE memory_id = id), json_array()))) "
...
def create_memster_memory(content, category='observation', tier='L2', tags=None):
    ...
    sql = f"INSERT INTO memories (content, category, tier, importance, created_at, accessed_at) "
    sql += f"VALUES ('{content_escaped}', '{cat_escaped}', '{tier_escaped}', 5.0, datetime('now'), datetime('now'))"
    cmd = f"sqlite3 '{MEMSTER_DB}' "{sql}""
    ...
    if mem_id and tags:
        for tag in tags:
            ...
            tag_sql = f"INSERT INTO memory_tags (memory_id, tag) VALUES ({mem_id}, '{tag_escaped}')"
            tag_cmd = f"sqlite3 '{MEMSTER_DB}' "{tag_sql}""
            ssh_command_raw(MEMSTER_HOST, MEMSTER_USER, tag_cmd)
...
def update_memster_memory(mem_id, content=None, category=None, tier=None, tags=None):
    ...
    updates.append("accessed_at = datetime('now')")
    ...
    if tags is not None:
        # Delete old tags
        del_sql = f"DELETE FROM memory_tags WHERE memory_id = {mid}"
        del_cmd = f"sqlite3 '{MEMSTER_DB}' "{del_sql}""
        ssh_command_raw(MEMSTER_HOST, MEMSTER_USER, del_cmd)
        
        # Insert new tags
        for tag in tags:
            tag_escaped = tag.replace("'", "''")
            tag_sql = f"INSERT INTO memory_tags (memory_id, tag) VALUES ({mid}, '{tag_escaped}')"
            tag_cmd = f"sqlite3 '{MEMSTER_DB}' "{tag_sql}""
            ssh_command_raw(MEMSTER_HOST, MEMSTER_USER, tag_cmd)
...
def delete_memster_memory(mem_id):
    ...
    edge_sql = f"DELETE FROM memory_edges WHERE source_id = {mid} OR target_id = {mid}"
    edge_cmd = f"sqlite3 '{MEMSTER_DB}' "{edge_sql}""
    ssh_command_raw(MEMSTER_HOST, MEMSTER_USER, edge_cmd)
    
    # Delete tags
    tag_sql = f"DELETE FROM memory_tags WHERE memory_id = {mid}"
    tag_cmd = f"sqlite3 '{MEMSTER_DB}' "{tag_sql}""
    ssh_command_raw(MEMSTER_HOST, MEMSTER_USER, tag_cmd)
    
    # Delete entities reference
    ent_sql = f"DELETE FROM entities WHERE memory_id = {mid}"
    ent_cmd = f"sqlite3 '{MEMSTER_DB}' "{ent_sql}""
    ssh_command_raw(MEMSTER_HOST, MEMSTER_USER, ent_cmd)
...
def add_memory_tag(mem_id, tag):
    ...
    check_sql = f"SELECT COUNT(*) FROM memory_tags WHERE memory_id = {mid} AND tag = '{tag_escaped}'"
    check_cmd = f"sqlite3 '{MEMSTER_DB}' '{check_sql}'"
    check_result = ssh_json_command(MEMSTER_HOST, MEMSTER_USER, check_cmd)
    
    try:
        count = int(check_result.get('output', '0').strip() or '0')
        if count > 0:
            return {'id': mid, 'tag': tag, 'added': False, 'reason': 'already exists'}
    except:
        pass
    
    sql = f"INSERT INTO memory_tags (memory_id, tag) VALUES ({mid}, '{tag_escaped}')"
    cmd = f"sqlite3 '{MEMSTER_DB}' "{sql}""
    result = ssh_command_raw(MEMSTER_HOST, MEMSTER_USER, cmd)
    
    if result.get('error'):
        return {'error': result['error']}
    return {'id': mid, 'tag': tag, 'added': True}

def remove_memory_tag(mem_id, tag):
    """Remove a tag from a memory."""
    mid = int(mem_id)
    tag_escaped = tag.replace("'", "''")
    sql = f"DELETE FROM memory_tags WHERE memory_id = {mid} AND tag = '{tag_escaped}'"
    cmd = f"sqlite3 '{MEMSTER_DB}' "{sql}""
    result = ssh_command_raw(MEMSTER_HOST, MEMSTER_USER, cmd)
    
    if result.get('error'):
        return {'error': result['error']}
    return {'id': mid, 'tag': tag, 'removed': True}
...
def get_related_memories(mem_id, limit=10):
    ...
    sql += "'created_at', m.created_at)) FROM memories m WHERE " + " AND ".join(conditions)
    sql += f" ORDER BY m.importance DESC, m.created_at DESC LIMIT {limit}"
    cmd = f"sqlite3 '{MEMSTER_DB}' '{sql}'"
    result = ssh_json_command(MEMSTER_HOST, MEMSTER_USER, cmd)
